...
import { ShareResult } from "@/components/dashboard/practice";
...
const SuccessTab = ({ id }: { id: string | string[] | undefined }) => {
  ...
  const searchParams = useSearchParams();
  const shareTab = searchParams.get("share") || "";
  const { user } = usePrivy();
    ...
    <>
      {!shareTab ? (
        <Card className="w-full max-w-[483px] mx-auto p-4 border-grey-500 space-y-6 flex flex-col items-center justify-center relative">
          ...
          <Button
            variant="link"
            onClick={() => router.push(`?tab=success&share=${examTest.slug}`)}
        ...
        <ShareResult score={score} examTest={examTest} />
      )}
    </>
  );
};
